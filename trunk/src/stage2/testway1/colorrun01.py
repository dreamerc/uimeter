...
# 載入外部視窗和初始化
    import Image
    im = Image.open("img/G1.tga")
    fakeicon1 = pygame.image.load("img/G1.tga")
    print im.format, im.size, im.mode
    points = [(50, 150), (im.size[0]+50, 150), (im.size[0]+150, im.size[1]+150), (50, im.size[0]+150)]
    moves = []
    hide = 0

    fakeicon2 = pygame.image.load("img/G2.tga")
    fakeicon3 = pygame.image.load("img/G3.tga")
...
# CSV 資料庫格式
    if csv_bool == 1:
        import csv
        csv_file = csv.writer(open("csv_file_c01.csv", "wb"),delimiter=' ',quotechar='|', quoting=csv.QUOTE_MINIMAL)
    else:
        pass
# 播放影片
